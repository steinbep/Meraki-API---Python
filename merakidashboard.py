...
def getclientsAll(apikey, networkid, perPage, suppressprint=False):
    calltype = 'Device Clients'
    geturl = '{0}/networks/{1}/clients?perPage={2}'.format(str(base_url), str(networkid), str(perPage))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
    }
...
# List the content filtering for a network
...
def updatecontentfilter(apikey, networkid, contentFilterUpdate, suppressprint=False):
	# confilter = [{"allowedUrlPatterns":["http://www.example.org","http://*.com.au"],"blockedUrlPatterns":["http://www.example.com","http://*.example"],"blockedUrlCategories":"%w(\"cf_001\", \"cf_002\")","urlCategoryListSize":"topSites"}]
...
# List the devices in a organization
# https://api.meraki.com/api_docs#list-the-devices-in-an-organization
def getorgdevices(apikey, organizationid, suppressprint=False):
    """
    ...
    """
    calltype = 'Organization'
    geturl = '{0}/organizations/{1}/devices'.format(str(base_url), str(organizationid))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
    }
...
def getMalwareSettings(apikey, networkid, suppressprint=False):
    ...
    geturl = '{0}/networks/{1}/security/malwareSettings'.format(str(base_url), str(networkid))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
    }
...
def updateMalwareSettings(apikey, networkid, mode=None, allowedUrls=None, allowedFiles=None, suppressprint=False):
    calltype = 'Malware Settings'
    puturl = '{0}/networks/{1}/security/malwareSettings'.format(str(base_url), str(networkid))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
    }
    putdata = {}
    if mode is not None:
        putdata['mode'] = format(str(mode))
    if allowedUrls is not None:
        putdata['allowedUrls'] = (allowedUrls)
    if allowedFiles is not None:
        putdata['allowedFiles'] = (allowedFiles)
